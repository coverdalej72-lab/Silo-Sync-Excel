...
function EobInfoPanel({ sheet, edits, farmName }: { sheet: SheetParsed; edits: Map<string, string>; farmName: string }) {
  ...
  const shareEmail = () => {
    ...
    const body = encodeURIComponent([
      ...
      `Total Feed Used      : ${totalFeedInStr ? fmt(totalFeedInStr) + " kg" : "—"}`,
  ...
  return (
    <div style={{ padding: "20px 20px 32px", fontFamily: "Inter,'Segoe UI',sans-serif", overflowY: "auto", height: "100%", boxSizing: "border-box" }}>
      ...
      {summary && (() => {
        ...
        // Total feed used = last batch leftover + all delivered this batch
        const totalFeedUsed  = lastBatchFeed + feedDelivered;
          ...
          <div style={{ background: "var(--pm-primary-soft)", border: "1px solid var(--pm-primary-border)", borderRadius: 10, padding: "14px 16px", marginBottom: 20 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(150px, 1fr))", gap: 10 }}>
              {[
                ...
                { label: "Total Feed Used", value: totalFeedUsed > 0 ? fmtN(totalFeedUsed)  + " kg" : "—", highlight: true },
                { label: "Feed on Hand",    value: feedOnHand    > 0 ? fmtN(feedOnHand)     + " kg" : "—" },
              ].map(({ label, value, highlight }) => (
                <div key={label} style={{ display: "flex", flexDirection: "column", gap: 2, ...(highlight ? { background: "var(--pm-primary-pale)", borderRadius: 6, padding: "6px 8px" } : {}) }}>
                  <div style={{ fontSize: 10, color: highlight ? "var(--pm-primary)" : "#666", textTransform: "uppercase", letterSpacing: 0.4, fontWeight: highlight ? 700 : 400 }}>{label}</div>
                  <div style={{ fontSize: 15, fontWeight: 700, color: highlight ? "var(--pm-primary)" : "var(--pm-primary-mid)" }}>{value}</div>
                </div>
              ))}
            </div>
          </div>
