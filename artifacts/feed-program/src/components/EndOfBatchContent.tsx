...
export function EndOfBatchContent({ sheet, edits, onEdit }: Props) {
  ...
  // Total Used = Last Batch Left + Total Delivered (same logic as green header)
  const lastBatchLeftNum   = parseFloat(lastBatchLeft.replace(/,/g, ""))  || 0;
  const totalPurchasedNum  = liveTotalPurchased > 0 ? liveTotalPurchased : (parseFloat(g(11, 18).replace(/,/g, "")) || 0);
  const feedLeftNum        = parseFloat(feedLeft.replace(/,/g, ""))       || 0;
  const totalUsedNum       = lastBatchLeftNum + totalPurchasedNum;
  const netConsumedNum     = totalUsedNum - feedLeftNum;
  const fmtKg = (n: number) => n > 0 ? n.toLocaleString() : "—";
  ...
  function buildEmailBody(): string {
    ...
    lines.push(`========================================`);
    lines.push(``);
    lines.push(`Last Batch Left:    ${lastBatchLeft || "—"} kg`);
    lines.push(`Total Delivered:    ${liveTotalPurchased > 0 ? liveTotalPurchased.toLocaleString() : (g(11, 18) || "—")} kg`);
    lines.push(`Total Used:         ${totalUsedNum > 0 ? totalUsedNum.toLocaleString() : "—"} kg`);
    lines.push(`Feed Left:          ${feedLeft || "—"} kg`);
    lines.push(`Net Consumed:       ${netConsumedNum > 0 ? netConsumedNum.toLocaleString() : "—"} kg`);
    lines.push(``);
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: 10, alignItems: "start" }}>
        ...
        <div style={{ background: "#fff", borderRadius: 10, border: "1px solid #e2e8f0",
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
            <tbody>
              {([
                { label: "Last Batch Left", value: lastBatchLeft || "—",                                          accent: false },
                { label: "Total Delivered", value: liveTotalPurchased > 0 ? liveTotalPurchased.toLocaleString() : (g(11, 18) || "—"), accent: false },
                { label: "Total Used",      value: fmtKg(totalUsedNum),                                           accent: false },
                { label: "Feed Left",       value: feedLeft || "—",                                               accent: false },
          ...
          {/* Net Consumed — highlighted row */}
          {netConsumedNum > 0 && (
            <div style={{ background: "linear-gradient(135deg, #1a5c36 0%, #2d7a50 100%)", padding: "9px 14px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              <span style={{ color: "rgba(255,255,255,0.85)", fontSize: 10, fontWeight: 700, textTransform: "uppercase", letterSpacing: 0.5 }}>Net Consumed</span>
              <div style={{ display: "flex", alignItems: "baseline", gap: 4 }}>
                <span style={{ color: "#fff", fontWeight: 800, fontSize: 16 }}>{netConsumedNum.toLocaleString()}</span>
                <span style={{ color: "rgba(255,255,255,0.65)", fontSize: 11 }}>kg</span>
              </div>
            </div>
          )}
        </div>
