      ...
      <div className="bg-card border border-primary/30 rounded-xl p-4 space-y-3">
        <div className="grid grid-cols-2 gap-3">
          ...
          <div>
            ...
            <input
              data-testid="input-edit-farm-url"
              type="url"
              value={form.apiUrl}
      ...
      <div className="flex items-center gap-1 shrink-0">
        ...
        {confirmDelete ? (
          <div className="flex items-center gap-1">
            ...
            <button
              data-testid={`btn-confirm-delete-${farm.id}`}
            ...
            <button
              data-testid={`btn-cancel-delete-${farm.id}`}
          ...
          <button
            data-testid={`btn-delete-${farm.id}`}
    ...
    <div className="bg-card border-2 border-primary/30 rounded-xl p-4 space-y-3">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          <input
            data-testid="input-new-farm-name"
            type="text"
            value={form.name}
            onChange={e => setForm(f => ({ ...f, name: e.target.value }))}
        ...
        <div>
          ...
          <input
            data-testid="input-new-farm-url"
            type="url"
            value={form.apiUrl}
            onChange={e => setForm(f => ({ ...f, apiUrl: e.target.value }))}
      ...
      <p className="text-[11px] text-muted-foreground">
        ...
        For remote farms, enter their full domain (e.g. <code className="font-mono bg-secondary px-1 rounded">https://farm2.yourdomain.com</code>).
      </p>
      <div className="flex gap-2 justify-end">
        <button
        ...
        <section className="bg-card border border-border rounded-2xl p-6">
          ...
          <div className="space-y-3 text-sm text-muted-foreground">
            <div className="flex gap-3">
              ...
              <p>Each <strong className="text-foreground">farm manager</strong> runs their own Farm Buddy app (Silo Tracker) on their device or server.</p>
            </div>
            <div className="flex gap-3">
              ...
              <p>Add each farm here with its <strong className="text-foreground">name</strong> and <strong className="text-foreground">API URL</strong> (the web address where their app is hosted).</p>
            </div>
            <div className="flex gap-3">
              <div className="w-6 h-6 rounded-full bg-primary/10 text-primary font-bold text-xs flex items-center justify-center shrink-0 mt-0.5">3</div>
              <p>The dashboard <strong className="text-foreground">fetches live data</strong> from each farm's API — feed on hand, today's readings, upcoming deliveries — and shows them side by side.</p>
            </div>
            <div className="flex gap-3">
              <div className="w-6 h-6 rounded-full bg-primary/10 text-primary font-bold text-xs flex items-center justify-center shrink-0 mt-0.5">4</div>
              <p>Click <strong className="text-foreground">Open App</strong> on any card to jump directly into that farm's own app in a new tab.</p>
            </div>
          </div>
        </section>
