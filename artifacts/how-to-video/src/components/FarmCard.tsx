...
export default function FarmCard({ name, apiUrl, data, onRefresh }: FarmCardProps) {
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 p-4 space-y-3">
        ...
        {progress && progress.sheds.length > 0 && (
          <div className="space-y-1.5">
            ...
            {progress.sheds.map((shed: ShedStatus) => {
              ...
              const maxSilo = shed.silos.reduce(
                (best: SiloStatus, s: SiloStatus) => (s.amountRemaining ?? 0) > (best.amountRemaining ?? 0) ? s : best,
                shed.silos[0]
              );
              return (
                <div
                  key={shed.shedGroupId}
                  data-testid={`shed-row-${shed.shedGroupId}`}
        ...
        {/* ── Next delivery ── */}
        {nextDelivery ? (
          <div className="flex items-center gap-3 px-3 py-2.5 rounded-xl bg-gold/10 border border-gold/20">
            <Truck className="w-4 h-4 text-gold shrink-0" />
            <div className="flex-1 min-w-0">
              <p className="text-[10px] font-bold uppercase tracking-wide text-gold/70">Next Delivery</p>
            ...
            <span className="text-xs font-bold text-gold shrink-0">{formatDate(nextDelivery.deliveryDate)}</span>
          </div>
        ) : (
      ...
      <div className="px-4 pb-4 pt-0 flex items-center justify-between">
        ...
        <a
          ...
          data-testid={`link-open-farm-${name.toLowerCase().replace(/\s+/g, "-")}`}
